...
<head>
<meta charset="utf-8"/>
<title>GuestBlogss | Backend</title>
...
<div class="content">
  <!-- BEGIN LOGIN FORM -->
  <?= form_open('adminaccess','class="login-form"'); ?>
    ...
    <div class="form-group">
      ...
      <div class="input-icon">
        ...
        <?= form_input('email', set_value('email'), ['class'=>"form-control placeholder-no-fix",'placeholder'=>"Email"]); ?>
      </div>
    </div>
     <?= form_error('email','<div class="adminloginerr text-danger">','</div>');?>
    <div class="form-group">
      <label class="control-label visible-ie8 visible-ie9">Password</label>
      <div class="input-icon">
        <i class="fa fa-lock"></i>
        <?= form_password('password', '', 'class="form-control placeholder-no-fix" placeholder="Password"'); ?>
      </div>
    </div>
    <?= form_error('password','<div class="adminloginerr text-danger">','</div>');?>
    <?=  ( $this->session->flashdata('userNotFound') != null) ? '<span class="text-danger">'.$this->session->flashdata('userNotFound').'</span>' : ""; ?>
  ...
  <form class="forget-form">
    ...
    <div class="alert alert-danger display-hide">
      <button class="close" data-close="alert"></button>
      <span class='forgotemailerror text-danger'></span>
    </div>
    <div class="form-group">
      <div class="input-icon">
        <i class="fa fa-envelope"></i>
        <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Email" name="forgetpassemail"/>
      </div>
    </div>
    ...
    <div class="form-actions">
      ...
      <button type="submit" id="forgetadminpass" class="btn green-haze pull-right">
...
<!-- BEGIN COPYRIGHT -->
<div class="copyright">
   2014 &copy; GuestBlogss. Admin Dashboard Template.
</div>
<!-- END COPYRIGHT -->
...
jQuery(document).ready(function($) {     
  ...
  $(document).on('click','#forgetadminpass',function(e){
    e.preventDefault();
    $('.display-hide').hide();
        ...
        success:function(data){
          if(data.success=='false' || data.success==false )
          {
            ...
            $.each(data.messages, function(k,v){
              $('.forgotemailerror').html(v);
              if($('.display-hide').hasClass('alert-success'))
              {
                $('.display-hide').removeClass('alert-success').addClass('alert-danger');
              }
              $('.display-hide').show();
            });
            }
            if(data.response)
            {
              $('.forgotemailerror').html(data.response);
              if($('.display-hide').hasClass('alert-success'))
              {
                $('.display-hide').removeClass('alert-success').addClass('alert-danger'); 
              }
          ...
          }else
          {
            ...
            {
              $('.forgotemailerror').html(data.response);
              if($('.display-hide').hasClass('alert-danger'))
              {
                $('.display-hide').removeClass('alert-danger').addClass('alert-success');
              }
              $('.display-hide').show();

            }
            $('input[name="forgetpassemail"').val('');
          }
